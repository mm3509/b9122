# Students: Global variable, constant throughout the script, to split level-1
# headings.
SPLITTER = "\n# "
...
def invert_markdown(markdown):
    ...
    ...
    AssertionError
    >>> invert_markdown("string without heading")
    Traceback (most recent call last):
    ...
    AssertionError
    >>> invert_markdown("## string with a level 2 heading")
    Traceback (most recent call last):
    ...
    AssertionError
    """
    ...
    assert markdown.startswith("# ")  # Or markdown[:2] == "# "
    markdown = markdown.strip("\n")

    # Clever solution: add a newline at the top of the file, so the splitting
    # works well in all cases.
    markdown = "\n" + markdown

    section_list = markdown.split(SPLITTER)

    if "" != section_list[0]:
        print("Bug! The first element should be an empty string")
        assert False
    section_list = section_list[1:]
    inverted_sections = section_list[::-1]

    inverted_markdown = SPLITTER[1:] + SPLITTER.join(inverted_sections)

    return inverted_markdown
